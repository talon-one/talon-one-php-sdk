...
```php
createCouponsAsync($applicationId, $campaignId, $newCouponCreationJob): \TalonOne\Client\Model\AsyncCouponCreationResponse
```
...
try {
    $result = $apiInstance->createCouponsAsync($applicationId, $campaignId, $newCouponCreationJob);
    print_r($result);
} catch (Exception $e) {
